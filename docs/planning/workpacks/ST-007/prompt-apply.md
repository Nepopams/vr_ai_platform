# Codex APPLY Prompt — ST-007: Partial Trust Rollout Runbook
...
- `docs/operations/partial-trust-runbook.md` (CREATE — directory `docs/operations/` does not exist yet, create it)

## Forbidden

- Any file under `routers/`, `app/`, `scripts/`, `tests/`, `agents/`, `graphs/`
- Any `.py` file
- `git commit`, `git push`
...
Create directory `docs/operations/` and file `docs/operations/partial-trust-runbook.md` with EXACTLY this content:
...
# 2. All env vars documented
grep "PARTIAL_TRUST_ENABLED" docs/operations/partial-trust-runbook.md | head -3
grep "PARTIAL_TRUST_SAMPLE_RATE" docs/operations/partial-trust-runbook.md | head -3
grep "PARTIAL_TRUST_TIMEOUT_MS" docs/operations/partial-trust-runbook.md | head -3
grep "PARTIAL_TRUST_RISK_LOG_PATH" docs/operations/partial-trust-runbook.md | head -3
grep "LLM_POLICY_ENABLED" docs/operations/partial-trust-runbook.md | head -3

# 3. Rollback section exists
grep -c "kill-switch\|Rollback" docs/operations/partial-trust-runbook.md

# 4. No secrets
grep -i "api.key\|secret\|password\|token" docs/operations/partial-trust-runbook.md || echo "NO SECRETS FOUND"
```
