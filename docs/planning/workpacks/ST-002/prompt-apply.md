# Codex APPLY Prompt — ST-002: Retroactive verification of shadow-router AC1-AC3

## Role

You are an implementation agent. Create and modify ONLY the files listed below.
...
- `docs/planning/initiatives/INIT-2026Q1-shadow-router.md` (MODIFY — status line only)
...
Create `docs/planning/epics/EP-001/verification-report.md` with EXACTLY this content:
...
In file `docs/planning/initiatives/INIT-2026Q1-shadow-router.md`, change line 5:
...
After creating/modifying the files, run:

```bash
# 1. Report exists
test -f docs/planning/epics/EP-001/verification-report.md && echo "OK" || echo "MISSING"

# 2. Initiative status updated
grep "Статус" docs/planning/initiatives/INIT-2026Q1-shadow-router.md

# 3. Shadow router tests still pass
python3 -m pytest tests/test_shadow_router.py -v
