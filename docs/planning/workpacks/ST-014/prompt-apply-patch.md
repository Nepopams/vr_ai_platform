# Codex APPLY PATCH — ST-014: Fix inline expected breaking command schema
...
Review found a must-fix: adding `expected` inline to fixture JSON files breaks `test_graph_execution.py` because `command.schema.json` has `additionalProperties: false`. Fixtures are valid CommandDTOs and cannot have extra keys.

**Fix**: move expected annotations from inline fixture JSON to a separate manifest file. Revert fixtures to clean CommandDTOs.

---

## Step 1: Revert `skills/graph-sanity/fixtures/commands/empty_text.json`

Replace the entire file with (remove `expected` key):
```json
{
...
## Step 2: Revert `skills/graph-sanity/fixtures/commands/unknown_intent.json`
...
## Step 3: Revert `skills/graph-sanity/fixtures/commands/minimal_context.json`
...
## Step 4: Revert `skills/graph-sanity/fixtures/commands/clarify_partial_shopping.json`
...
## Step 5: Revert `skills/graph-sanity/fixtures/commands/clarify_ambiguous_intent.json`
...
## Step 6: Create `skills/graph-sanity/fixtures/commands/clarify_expected.json`

Create this NEW manifest file:
```json
[
  {
    "command_id": "cmd-wp000-005",
    ...
    "expected_missing_fields": ["text"],
    "notes": "Whitespace-only text triggers clarify for text"
  },
  {
    "command_id": "cmd-wp000-006",
    ...
    "notes": "No keyword match triggers clarify for intent"
  },
  {
    "command_id": "cmd-wp000-007",
    "fixture_file": "minimal_context.json",
    "expected_action": "start_job",
    "expected_missing_fields": null,
    "notes": "sделай matches TASK_KEYWORDS -> create_task -> start_job"
  },
  {
    "command_id": "cmd-wp000-015",
    "fixture_file": "clarify_partial_shopping.json",
    "expected_action": "clarify",
    "expected_missing_fields": ["item.name"],
    "notes": "Купи matches shopping intent but no item name extractable"
  },
  {
    "command_id": "cmd-wp000-016",
...
## Step 7: Replace `scripts/analyze_clarify_quality.py`
...
def load_fixtures_with_expected(
    ...
    for path in sorted(fixtures_dir.glob("*.json")):
        if path.name == "clarify_expected.json":
            continue
...
def main() -> None:
    ...
    manifest_path = args.manifest or (args.fixtures_dir / "clarify_expected.json")
...
## Step 8: Replace `tests/test_clarify_measurement.py`
...
MANIFEST_PATH = FIXTURES_DIR / "clarify_expected.json"
...
# 2. Full test suite (must be 202 passed, 0 failed)
...
Expected: ALL tests pass (202/202), no secrets, self-test ok.
...
| File | Action |
|------|--------|
| `skills/graph-sanity/fixtures/commands/empty_text.json` | Remove `expected` key (Step 1) |
| `skills/graph-sanity/fixtures/commands/unknown_intent.json` | Remove `expected` key (Step 2) |
| `skills/graph-sanity/fixtures/commands/minimal_context.json` | Remove `expected` key (Step 3) |
| `skills/graph-sanity/fixtures/commands/clarify_partial_shopping.json` | Remove `expected` key (Step 4) |
| `skills/graph-sanity/fixtures/commands/clarify_ambiguous_intent.json` | Remove `expected` key (Step 5) |
| `skills/graph-sanity/fixtures/commands/clarify_expected.json` | Create new manifest (Step 6) |
| `scripts/analyze_clarify_quality.py` | Replace with manifest-based version (Step 7) |
| `tests/test_clarify_measurement.py` | Replace with manifest-based tests (Step 8) |
...
- Fixture JSON files are clean CommandDTOs — NO `expected` key
- `skills/graph-sanity/scripts/run_graph_suite.py` — NOT modified
- `tests/test_graph_execution.py` — NOT modified
