# Codex APPLY — ST-017: Contract Governance Runbook

## Role
You are an implementation agent. Create exactly the files specified below.

## Allowed files (whitelist)
- `docs/contracts/CONTRACT-GOVERNANCE-RUNBOOK.md` (CREATE)

## Forbidden
- Any file outside `docs/contracts/`
- Any code changes
- Network access
- `git commit/push`
...
## Step 1: Create `docs/contracts/CONTRACT-GOVERNANCE-RUNBOOK.md`
...
for section in "Breaking vs Non-Breaking" "Non-Breaking Change Workflow" "Breaking Change Workflow" "Version Bump" "CI Checks Explained" "PR Checklist"; do
  grep -qi "$section" docs/contracts/CONTRACT-GOVERNANCE-RUNBOOK.md && echo "OK: $section" || echo "MISSING: $section"
done

# No secrets
grep -rn 'sk-\|api_key\|secret' docs/contracts/CONTRACT-GOVERNANCE-RUNBOOK.md; echo "Secrets check done"
```

Expected: file exists, all 6 sections found, no secrets.
